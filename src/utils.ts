...
export const setPctChange = async (
	...
	if (!Array.isArray(sales)) {
		console.error(
			"Cannot calculate percentage change from a non-array sales response",
			{ id },
		);
		return 0;
	}
	const cutoffs = timeframes.map(
		(tf) => currentHeight - Math.floor(tf.value * 144),
	);
...
export const fetchExchangeRate = async (): Promise<number> => {
	...
	const exchangeRateData = (await fetchJSON(
...
export const fetchTokensDetails = async <T extends BSV20Details | BSV21Details>(
	tokenIDs: string[],
	assetType: AssetType,
): Promise<T[]> => {
	const d: T[] = [];
	...
	switch (assetType) {
		case AssetType.BSV20:
			...
			for (const tick of tokenIDs) {
				...
				d.push(details);
				await redis.set(
					`token-${assetType}-${tick.toLowerCase()}`,
