 ...
 */
export async function addUsdQuotesToMarketData<
	T extends { price: number; marketCap?: number },
>(items: T[]): Promise<(T & { quotes?: QuotesObject })[]> {
	try {
			...
			quotes: {
				...
				USD: {
					...
					...(item.marketCap !== undefined && {
						market_cap: item.marketCap * bsvUsdRate,
					}),
				},
			},
		}));
 ...
 */
export async function addUsdQuotesToSingleItem<
	T extends { price: number; marketCap?: number },
>(item: T): Promise<T & { quotes?: QuotesObject }> {
	try {
		const bsvUsdRate = await getBsvUsdRate();
 ...
 */
export async function addUsdToBalances<
	T extends { price?: number; value?: number; amount?: number },
>(
	balances: T[],
): Promise<
	(T & { price_usd?: number; value_usd?: number; value_bsv?: number })[]
> {
	try {
		const bsvUsdRate = await getBsvUsdRate();
