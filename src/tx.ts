...
import { API_HOST, ordPk, payPk } from "./constants";
...
export const createAirdropTx = async (
	...
	if (!ordPk || !payPk) {
		throw new Error(
			"ORDPK and PAYPK environment variables must be set for airdrops",
		);
	}
	...
	const tokenUtxos = inputTokens.map((token) => {
		if (!token.script) {
			throw new Error(
				`Token UTXO ${token.txid}:${token.vout} is missing script`,
			);
		}
		return {
			satoshis: 1 as const,
	...
	if (paymentUtxos.length > 0) {
		...
		pipeline.hdel(
			"pay-utxos",
			`${usedPaymentUtxo.txid}_${usedPaymentUtxo.vout}`,
		);
	}
