...
import {
	findMatchingKeys,
	findMatchingKeysWithOffset,
	findOneExactMatchingKey,
} from "./db";
import {
	fetchV1Tickers,
	fetchV2Tickers,
...
import {
	addUsdQuotesToMarketData,
	addUsdQuotesToSingleItem,
	addUsdToBalances,
} from "./services/usd-quotes";
...
const app = new Elysia()
	...
	.get(
		...
		async ({ params }) => {
			...
			return results.sort((a, b) =>
				a.id.toLowerCase() === id ? -1 : b.id.toLowerCase() === id ? 1 : 0,
			);
		},
		{
		...
		async ({ set, body }) => {
			...
			for (const id of ids) {
				...
				const cached = ((await redis.get(
					`token-${AssetType.BSV20}-${id.toLowerCase()}`,
				)) || "{}") as string;
				const parsed = JSON.parse(cached) as MarketDataV1;
	...
	.get(
		"/collection/:collectionId/market",
	...
	.get(
		...
		async ({ set, query }) => {
			...
			try {
				...
				const collections = await findMatchingKeysWithOffset(
					redis,
					"collection",
					"",
					AssetType.Ordinals,
		...
		async ({ params, query, set }) => {
			...
			try {
				...
				const response = await fetch(
					`${API_HOST}/inscriptions/${collectionId}`,
				);
				const collectionData = (await response.json()) as Record<
					string,
					unknown
				>;
				...
				if (response.status === 200) {
					...
					const stats = await fetchJSON(
						`${API_HOST}/collections/${collectionId}/stats`,
					);
					collectionData.stats = stats;

					await redis.hset(
						`collection-${AssetType.Ordinals}`,
						collectionId,
				...
				try {
					const sales = await fetchCollectionSales(
						collectionId,
						Number.parseInt(offset || "0", 10),
						limit ? Number.parseInt(limit, 10) : NUMBER_OF_ITEMS_PER_PAGE,
					);
	...
	.get(
		...
		async ({ set, params, query }) => {
			...
			const {
				...
				offset = "0",
				sort = SortBy.MostRecentSale,
				dir = "asc",
			} = query;
			console.log({ query });

			try {
				const marketData = await fetchShallowMarketData(
				...
				const sortedData = marketData.sort(
					(a: MarketDataV1 | MarketDataV2, b: MarketDataV1 | MarketDataV2) => {
						...
						const compareByNumber = (
							propName: keyof (MarketDataV1 | MarketDataV2),
						): number => {
							const aProp = Number(
								(a as MarketDataV1 | MarketDataV2)[propName] || 0,
							);
							const bProp = Number(
								(b as MarketDataV1 | MarketDataV2)[propName] || 0,
							);
							return aProp - bProp;
						};
						...
						const compareFunction =
							compareFunctions[sortMethod] || compareByMostRecentSale;
						return compareFunction() * sortDirection;
					},
				);

				// Add USD quotes to the sorted data
				return await addUsdQuotesToMarketData(
					sortedData as (MarketDataV1 | MarketDataV2)[],
				);
			} catch (e) {
	...
	.get(
		...
		},
		{
			...
			query: t.Object({
				...
				type: t.Optional(
					t.Union([t.Literal("bsv20"), t.Literal("bsv21")], {
		...
		async ({ set }) => {
			...
			try {
				...
				const resp = await fetchJSON(
					`${API_HOST}/txos/search/unspent?q=${b64}`,
				);
			...
			switch (template) {
				...
				case "3": {
					...
					addresses = await Promise.all(
						users.map(async (userKey: string) => {
	...
	.post(
		...
		async ({ body }) => {
			...
			const { rawTx, txid, spend } = await createAirdropTx(
				to,
				50,
				paymentUtxos,
				inputTokens,
	...
	.get(
		...
		async ({ params, set }) => {
			...
			const resp = await fetchJSON<Balance[]>(
				`${API_HOST}/bsv20/${params.address}/balance`,
			);
			if (!resp) {
				set.status = 404;
				return [];
			...
			const enriched: EnrichedBalance[] = await Promise.all(
				resp.map(async (b) => {
					const cached = await redis.get(
						`token-${b.tick ? AssetType.BSV20 : AssetType.BSV21}-${b.tick || b.id}`,
					);
					if (!cached) {
						return { ...b, price: 0 };
					}
	...
	.get(
		...
		async ({ params, set, query }) => {
			...
			const fundingKey =
				key === "bot" ? process.env.PAYPK : process.env.BROADCAST_FUNDING_WIF2;
			if (!fundingKey) {
...
const fetchMarketData = async (assetType: AssetType, id: string) => {
	...
	switch (assetType) {
		case AssetType.BSV20: {
			...
			detailedTokensV1 = await fetchTokensDetails<BSV20Details>(
				[id],
				assetType,
			);
		...
		case AssetType.BSV21: {
			...
			detailedTokensV2 = await fetchTokensDetails<BSV21Details>(
				[id],
				assetType,
			);
...
const findMarketData = async (assetType: AssetType, term: string) => {
	...
	switch (assetType) {
		case AssetType.BSV20: {
			...
			const results = await findMatchingKeys(
				redis,
				"autofill",
				term,
				assetType,
...
const fetchShallowMarketData = async (
	assetType: AssetType,
	offset = 0,
	limit = 20,
) => {
	switch (assetType) {
		case AssetType.BSV20: {
			...
			const [, ticks] = await redis.zscan(
				`included-${AssetType.BSV20}`,
				offset,
				"COUNT",
				limit,
			);
			...
			for (let i = 0; i < ticks.length; i += 2) {
				...
				const cachedListings = await redis.keys(
					`listings-${AssetType.BSV20}-${tick}`,
				);
		...
		case AssetType.BSV21: {
			...
			const [, ids] = await redis.zscan(
				`included-${AssetType.BSV21}`,
				offset,
				"COUNT",
				limit,
			);
