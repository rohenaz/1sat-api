...
const fetchV1TickerNames = async (
	offset: number,
...
const fetchV2TickerNames = async (
	offset: number,
	resultsPerPage: number,
	included: boolean,
) => {
...
export const loadAllV1Names = async (): Promise<void> => {
	...
	while (true) {
		...
		for (const result of results) {
			...
			await redis.hset(
				`autofill-${AssetType.BSV20}`,
				result.tick.toLowerCase(),
	...
	console.log(
		"All tickers cached for autofill",
		includedCount,
		unincludedCount,
	);
};
...
export const loadIncludedV2Names = async (): Promise<void> => {
	...
	while (true) {
		...
		for (const result of results) {
			...
			await redis.hset(
				`autofill-${AssetType.BSV21}`,
				`${result.tick.toLowerCase()}-${result.id.toLowerCase()}`,
				JSON.stringify(result),
			);
		}
		if (results.length < resultsPerPage) {
	...
	console.log(
		"All v2 tickers cached for autofill",
		includedCount,
		includedCount,
	);
};
...
export const fetchV2Tickers = async () => {
	...
	const info = await fetchChainInfo();
	return await loadV2TickerDetails(
		tickersV2.filter((t) => {
...
export const loadV1TickerDetails = async (
	tickersV1: BSV20V1[],
	info: ChainInfo,
) => {
	...
	for (const t of tickersV1) {
		...
		const cached = ((await redis.get(
			`token-${AssetType.BSV20}-${tick.toLowerCase()}`,
		)) || "{}") as string;
		...
		const autofillData = await redis.hget(
			`autofill-${AssetType.BSV20}`,
			ticker.tick.toLowerCase(),
		);
		if (autofillData) {
...
export const loadV2TickerDetails = async (
	tickersV2: BSV21[],
	info: ChainInfo,
) => {
...
function updateTickerWithPow20Data(
	ticker: MarketDataV2,
	pow20Data: Partial<BSV21>,
): void {
...
function updateTickerWithLTMData(
	ticker: MarketDataV2,
	ltmData: Partial<BSV21>,
): void {
...
async function updateIncludedCache(
	id: string,
	ticker: MarketDataV2,
): Promise<void> {
	if (ticker.included) {
